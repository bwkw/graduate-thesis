...
本研究では、低次の多項式として5次の多項式を採用する。
近似の精度$A$を、各数密度$\rho$における圧力の測定値$P$と近似値$P'$の百分率誤差の絶対値の平均として定義すると、
近似の精度は、

\large
\begin{equation}
    A=\frac{\sum_{i=1}^N\left(|\frac{P'-P}{P}\times100|\right)}{N} \label{eq:}
\end{equation}
\normalsize
と表される。
尚、本研究における低次の多項式近似とは、
...
\large
\begin{equation}
    P=kT\rho+b\rho^2+c\rho^3+\cdots\label{eq:}
\end{equation}
\normalsize
と表したものであり、
