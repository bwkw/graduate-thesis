...
\]
\normalsize
が与えられているとし、簡単のため、$y$は平均が$\bm{0}$となるように正規化してあるとすると、$y=f(x)$という関数$f$が平均0のガウス過程
...
\normalsize
から生成されているとする。
尚、$k(x,x')$は、カーネルと呼ばれるもので入力$x$の間の類似度を測るために使われるものである\cite{Gauss-machine-learning}。
...
\end{eqnarray}
\normalsize
とし、$x_1\,,\,{\cdots}\,,\,x_N$および、$x^*$計算される$(N+1)×(N+1)$次元のカーネル行列を$\bm{K'}$とすると、
これら全体がまたガウス分布に従うので、
...
以上から、ガウス過程回帰は、\cite{Gaussian-Processes-for-Machine-Learning}で開発された導出に従って、観測された$\bm{y}$と予測された$\bm{y}^*$の同時分布が
...
={\mathcal{N}}(\bm{k}_*^T\bm{K}^{-1}\bm{y}\,,\,\bm{k}_{**}-\bm{k}_*^T\bm{K}^{-1}\bm{k}_*)  \nonumber
\end{eqnarray}
\normalsize
と表わされる。
