...
\large
\begin{equation}
f(x)=\sum_{i=0}^m a_ix^i\label{eq:poly­nomial}
\end{equation}
\normalsize
で表すとし、
測定で得られた、$x_i$\,,\,$y_i$の組を
...
\large
\begin{equation}
S=\sum_{i=0}^n (y_i-f(x_i))^2\label{eq:residual-error}
\end{equation}
\normalsize
...
\ref{method-sec:Gauss}では、ガウス過程回帰を用いて、WCAポテンシャルの状態方程式を効率的に求める。
\ref{method-subsec:Gauss}では、ガウス過程回帰の説明を示し、\ref{}

\subsection{Gaussian Process Regression(ガウス過程回帰)}\label{method-subsec:Gauss}
