...
実存気体の状態方程式を再現しようとする試みは、1901年にHeikeが提唱したビリアル展開\cite{eq:virial}、
1940年にManson、George、Rubinが提唱したBenedict-Webb-Rubin(BWR)方程式\ref{eq:BWR}、1973年にJacobsen、Stewartが提唱したmodified-Benedict-Webb-Rubin(mBWR)方程式
...
本研究では、WCAポテンシャルにおける圧力$P$を数密度$\rho$を含む項で書き下す状態方程式を推定しながら、相転移を効率的に確認することでWCAポテンシャルにおける相図を作成することを目標とする。
WCAポテンシャルは、温度一定のまま数密度を高くしていくと、ある密度で流体相から固体相への相転移を起こす。
...
相転移近傍を効率的に探索するようなアルゴリズムを構築することにより、WCAポテンシャルの相図を効率的に作成する。
...
まず、\ref{results-sec:WCA-press}節のように数密度$\rho=0.0〜15.0$における圧力$P$を効率的に推定することを試みた。
