...
分子動力学計算において頻繁に用いられるモデルの一つにLennard-Jones\,(以下LJ)ポテンシャルと呼ばれるものが存在する。
...
\large
\begin{equation}
\rho^*=\frac{kT}{\varepsilon}\label{eq:T}
\end{equation}
\normalsize
...
\large
\begin{equation}
P^*=\frac{kT}{\varepsilon}\label{eq:T}
\end{equation}
\normalsize
...
図{\ref{fig:dis-poen}にLJポテンシャルとWCAポテンシャルにおける原子間距離$r$と原子間のポテンシャルエネルギー$E$を示す。
...
$r>2^{\frac{1}{6}}{\approx}1.12246$でポテンシャルエネルギーが0になっていることが確認できる。
...
以下では、WCAポテンシャルにおける相図を確認する。
式(\ref{eq:wca})に示したWCAポテンシャルにおける相図の概念図を図\ref{fig:wca-phase-diagram}に示す。
...
図\ref{fig:wca-phase-diagram}で示したWCAの相図を見てもわかる通り、中密度あたりで相転移が起こることから、
中密度で圧力の振る舞いが急激に変化すると考えられる。
以上から、本研究では、lammpsによるシミュレーションで測定されたWCAポテンシャルの圧力の振る舞いを低密度(原子間の相互作用がない)、高密度、中密度において確認することを目的とする。
