...
本研究では、数密度0.01〜15.0に0.01ずつ値を取りlammpsによるシミュレーションで圧力を測定したが、カットオフ距離1.12246のWCAポテンシャルにおいて低密度側(再隣接原子間距離$<$1.12246)では、原子間の相互作用(斥力作用)はないことから、
...
原子の密度が高くなればなるほど、式(\ref{eq:Theoritical-pressure})の第二項で考慮するものは、再隣接原子、第二隣接原子、第三隣接原子・・・と増えていくが、
式(\ref{eq:Theoritical-pressure})の第二項への寄与はほとんどが再隣接原子であることから、本研究では、再隣接原子の寄与のみを考えるものとする。

以上を踏まえて、本研究では、lammpsによるシミュレーションで測定されたWCAポテンシャルの圧力が式(\ref{eq:Theoritical-pressure})で書けることを確認することを目的とする。
...
まず、\ref{method-sec:WCA-press}の全体結果を以下に示す。
\begin{figure}[htbp]
    \begin{center}
...
lammpsによるシミュレーションで測定されたWCAポテンシャルの圧力が式(\ref{eq:Theoritical-pressure})で書ける
