...
\ref{method-WCA-pressure}節では、式(\ref{eq:wca})に示したWCAポテンシャルにおける圧力の測定を分子動力学プログラムLarge-scale Atomic/Molecular Massively Parallel Simulator\,(LAMMPS)\cite{lammps}によるシミュレーションを用いて行う。
...
これは、Heikeによって1901年に提唱されたもので、多くの研究で利用されている\cite{virial-Heike}。
...
そこで、本研究では機械学習により、より多くの候補からWCAポテンシャルにおける状態方程式を推定する方が効果的であると言える。
最終的には、機械学習が推定してきた項の物理的意味を考察することが目標であるが、まず本研究では最小二乗法による低次の多項式近似により状態方程式を推定し、
展開係数が先行研究と矛盾しないことを確認する。
...
上記で示した最小二乗法を適用し、密度に関する低次の多項式で近似する。
その後、近似した方程式の精度を調べる。
